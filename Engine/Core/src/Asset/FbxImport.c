...
void init_asset_importer(AssetImporter* importer) {
	...
	map_init(&importer->fbx_object_map);

	importer->asset_info_import_count = 0;
	importer->assets_infos = NULL;

}
void destroy_asset_importer(AssetImporter* importer) {
	stack_reset(&importer->stack);
	arena_free(&importer->mem);
	map_destroy(&importer->fbx_object_map);
	sb_free(importer->assets_infos);
}
...
AssetInfo export_static_mesh(AssetImporter* importer,
	...
	importer->asset_info_import_count++;
	sb_push(importer->assets_infos, asset_info);
...
void static fbx_process_objects_node(AssetImporter* importer, FBX_Node* node) {
	...
	while (obj_node != NULL) {
		...
		if (strcmp(obj_node->name, "Geometry") == 0) {
			...
			map_put(&importer->fbx_object_map, id, object);
...
void static fbx_process_connections_node(AssetImporter* importer, FBX_Node* node) {
	...
	while (connection_node != NULL) {
		...
		if (strcmp(connection_type, "OO") == 0) {
			...
			MapResult<FBX_Object> child_result = map_get(&importer->fbx_object_map, child_object_id);

			if (child_result.found) {
				FBX_Object child_object = child_result.value;
				
				// Base pos,rot,scale for root
				Vec3f local_pos = Vec3f(0, 0, 0);
				Vec3f local_rot = Vec3f(0, 0, 0);
				Vec3f local_scale = Vec3f(1, 1, 1);
				...
				switch (child_object.type) {
					case FBX_Object_Type_Geometry: {
						...
						if (parent_object_id == 0) {
							...
							MapResult<FBX_Object> parent_result = map_get(&importer->fbx_object_map, parent_object_id);
							FBX_Object parent_object;

							

							if (parent_result.found) {
								parent_object = parent_result.value;
								...
								local_pos = parent_model->local_translation;
								local_rot = parent_model->local_rotation;
								local_scale = parent_model->local_scaling;


								str_length = parent_model->name_length;
								name = parent_model->name;

							} else {
								// There was no parent object found
								assert_fail();
							}
					...
					case FBX_Object_Type_Model: {
						...
						local_pos = child_object.model->local_translation;
						local_rot = child_object.model->local_rotation;
						local_scale = child_object.model->local_scaling;
						break;
					}
				}
				...
				// No child found, skip this object for now
				//assert_fail();
			}
...
void static fbx_process_node(AssetImporter* importer, FBX_Node* node) {
	...
	if (strcmp(node->name, "Objects") == 0) {
		fbx_process_objects_node(importer, node);
	} else if (strcmp(node->name, "Connections") == 0) {
		fbx_process_connections_node(importer, node);
	}
...
static FBX_Node fbx_parse_node(AssetImporter* importer, void* buffer, FILE* file) {
	...
	while (current_offset < node.end_offset - 13) {
		FBX_Node temp = fbx_parse_node(importer, buffer, file);
...
void import_fbx(AssetImporter* importer, const char* filename, bool y_is_up) {
	...
	for (;;) {
		...
		FBX_Node node = fbx_parse_node(importer, buffer, file);
		if (node.end_offset == 0) break;
		fbx_process_node(importer, &node);
